...
def startTimer(window):
    global timer
    pomWindow = Window(window, title="Pomodoro Timer")
    pomWindow.height = 100
    pomWindow.width = 250
    currentTimeLeft = 25
    timerTitle = Text(pomWindow, text="Pomodoro Timer")
    timer = Text(pomWindow, text=currentTimeLeft)
    timer.repeat(60000, reduceTime)
...
title = Text(app, text="To Do List")
pomodoroTimer = PushButton(app, text="Pomodoro Timer", command=lambda:startTimer(app))
